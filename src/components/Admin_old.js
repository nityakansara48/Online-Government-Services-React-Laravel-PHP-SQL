...
function Admin_old() {
  ...
  const getAllData = async () => {
    ...
    resp = await fetch('http://localhost/siremar_data/Get_inspector.php');
    setInspectorData(await resp.json());
    resp = await fetch('http://localhost/siremar_data/Get_flight.php');
  ...
  const [delInspector, setDelInspector] = useState({
    id: "",
    table: ""
  });
  ...
  if (useState.authorised === "false") {
    console.log(useState.authorised);
  ...
  const addInspectorRow = (e) => {
    ...
    axios.post("http://localhost/siremar_data/Add_resident.php", { userData: addInspector }).then(
      res => {
        ...
        if (res.data.message === 'Success') {
          alert(res.data.response_msg);
          handleInspectorReset("Inspector Registered Successfull.");
        }
        else {
          alert(res.data.response_msg);
    ...
    <>
      <div>
        <ul>
          ...
          <li className='left'><Link onClick={gotoDashboard}>Admin Dashboard</Link>&nbsp;</li>
        </ul>
      </div>

      <div className='ScrollStyle_admin'>
          ...
          <table className="dashboardTable">
            ...
            <tr>
              ...
              <td>
                ...
                <form onSubmit={deleteRow}>
                  <p>Delete Inspector Id: <input type="text" name="Inspector" value={delInspector.id} onChange={handleDelete} />   <input type="submit" value="Delete" /></p>
                </form>
              </td>
            </tr>
            <tr>
