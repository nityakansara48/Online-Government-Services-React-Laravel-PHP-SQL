...
function InspectorFlight() {
  ...
  const addFlightRow = (e) => {
    ...
    axios.post("http://localhost:8000/flight", addFlight).then(
      (res) => {
        console.log(
          "Response Data after Add Flight: " + Object.values(res.data)
        );
  ...
  const handleDeleteFlight = (e) => {
    ...
    axios
      .post("http://localhost:8000/flight/delete/" + delFlight.flight_id)
      .then((res) => {
        if (res.data.message === "Success") {
        ...
        <table className="dashboardTableData">
          ...
          <tr>
            ...
            <td>
              <input
                name="from_city"
                value={addFlight.from_city}
                onChange={handleFlightRow}
                className="td-w"
            ...
            <td class="link" onClick={addFlightRow}>
              Add
            </td>
          </tr>
        </table>
      ...
      <form onSubmit={handleDeleteFlight}>
        <p>
          ...
          <input
            type="text"
            name="flight_id"
            value={delFlight.flight_id}
            onChange={handleDelFlight}
          />{" "}
...
export default InspectorFlight;
